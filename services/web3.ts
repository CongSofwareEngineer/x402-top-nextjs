...
import { createPublicClient, http, type Address, type Hash, type Hex, type TypedData, type TypedDataDefinition } from 'viem'
import { privateKeyToAccount } from 'viem/accounts'
import { authorizationTypes } from '@x402/evm'

import { NETWORK } from '@/config/x402'
import { decryptData } from '@/utils/argon2'
import { CHAIN_APP } from '@/config/appkit'
...
export interface TransferWithAuthorizationParams {
  ...
  validBefore?: bigint
  nonce?: Hex
  domain?: TypedDataDefinition<TypedData, 'TransferWithAuthorization'>['domain']
}
 ...
 */
export const BASE_CHAIN_ID = CHAIN_APP.id // 8453

if (NETWORK !== `eip155:${BASE_CHAIN_ID}`) {
  throw new Error(`Unsupported network: ${NETWORK}. Only Base (eip155:${BASE_CHAIN_ID}) is allowed.`)
}
...
export class Web3 {
  ...
  async decodePrivateKey(): Promise<Hex> {
    ...
    if (!/^[0-9a-fA-F]{64}$/.test(raw)) {
      throw new Error('Decoded PRIVATE_KEY_ENCODE is not a valid 32-byte private key')
    }
   ...
   */
  async signTypedData<const TTypedData extends TypedData | Record<string, unknown>, TPrimaryType extends string = string>(
